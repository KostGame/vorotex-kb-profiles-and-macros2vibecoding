...
Require(config.States.Running.Mode != K15LightingMode.TetrisBlocks,
    "Tetris must not be a default state effect.");
...
Require(parsed.ProfileSwitch.DurationSeconds == 2, "TOML round-trip must preserve profile-switch duration.");
...
Require(K15HidProtocol.ModeCode(K15LightingMode.TetrisBlocks) == 0x86,
    "Tetris mapping remains available for explicit experiments.");
...
Console.WriteLine("State reducer + TOML/profile-color renderer + HID protocol smoke tests: PASS");
