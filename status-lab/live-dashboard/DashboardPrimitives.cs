...
public static class EventSanitizer
{
    ...
    {
        try { using var doc = JsonDocument.Parse(line); var r = doc.RootElement; var source = Text(r, "source"); var name = Text(r, "event"); if (!DateTimeOffset.TryParse(Text(r, "timestampUtc"), out var timestamp) || !AllowedShape(r, source) || !AllowedEvent(source, name)) return null;
