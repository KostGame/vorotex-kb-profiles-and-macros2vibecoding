...
`ordinary Codex Desktop -> User CODEX_CLI_PATH -> K15.CodexBridge.WindowsAdapter.exe -> approval-wrapper.mjs -> exact current-Desktop-compatible codex.exe`
...
The manifest child pair remains the reviewed activation baseline and offline/rollback fallback. At actual Desktop process launch, the adapter does not let that baseline force an updated Desktop through an obsolete backend. If the configured child is absent or belongs to the standard `%LOCALAPPDATA%\OpenAI\Codex\bin` tree, the adapter reads the package root of the currently running `OpenAI.Codex_...\app\ChatGPT.exe` and mirrors the stock Windows core relocation contract for exactly four package files: `codex.exe`, `codex-code-mode-host.exe`, `codex-windows-sandbox-setup.exe`, and `codex-command-runner.exe`.

The generation directory name is the first 16 lowercase hex characters of SHA-256 over each exact filename, NUL, lowercase file SHA-256, NUL in that order, matching the current Desktop implementation. An existing generation is accepted only when it contains exactly those four byte-identical regular files. If the generation is absent or contains only expected names but is incomplete/mismatched, the adapter materializes the exact current package files through same-root staging, verifies every copy, and atomically renames staging into the generation. A target generation with any foreign entry fails closed and is preserved. Old valid generations are not removed by the bridge. The selected child path and SHA override remain process-local; the persistent manifest and User/Machine environment are not silently repinned. WindowsApps is never modified.
...
After active writes, `Enable` independently rereads all seven managed variables and requires the exact active presence/value state before reporting `ACTIVE=YES`. `Disable` restores and independently rereads the recorded presence/value baseline, broadcasts the environment change, and only then removes activation state. It can restore both legacy activation-state v2 and current v3. A failed postcheck reports only the variable name, expected/current presence, and whether the value matched; it never prints the value. Failed postchecks or broadcasts retain retryable state. `USER_ENV_MUTATED=YES` is emitted whenever this invocation actually writes or deletes a User-environment value.

## Update and rollback semantics

Every executable/module in the activation bundle, including Node and `runtime-process-authority.mjs`, has an explicit review pin. Replacement-in-place of the adapter, either wrapper, bridge core, authority module, Node, `codex.exe`, or `codex-code-mode-host.exe` causes `Validate` and `Enable` to fail closed before any User environment mutation. The activation manifest is never silently rewritten or repinned. Separately, a running Desktop invocation may use or materialize the stock generation derived from the current package's exact four-file core bundle. This preserves cold-start compatibility after a Desktop auto-update without treating an arbitrary LocalAppData directory as trusted configuration.
...
The health state remains an activation-maintenance signal. After a Desktop update, `UPDATE_REVALIDATION_REQUIRED` may coexist with a functioning bridge process that has selected the exact package-identical current runtime for that invocation. Owner repin remains useful to bring the persisted activation baseline back to `HEALTHY`, but it is no longer required merely to preserve native Desktop/plugin functionality.
