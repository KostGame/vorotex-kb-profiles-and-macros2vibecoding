...
The manifest child pair remains the reviewed activation baseline and offline/rollback fallback. At actual Desktop process launch, the adapter does not let that baseline force an updated Desktop through an obsolete backend. If the configured child is absent or belongs to the standard `%LOCALAPPDATA%\OpenAI\Codex\bin` tree, the adapter reads the package root of the currently running `OpenAI.Codex_...\app\ChatGPT.exe`, hashes its packaged `codex.exe` and `codex-code-mode-host.exe`, and selects only a LocalAppData runtime generation whose pair is byte-identical. The resulting child path and SHA override exist only in the adapter's child-process environment. No manifest, User/Machine environment, package file, or runtime file is rewritten. If an exact matching pair is unavailable, launch fails closed instead of silently using an incompatible generation.
