...
. (Join-Path $PSScriptRoot '..\..\codex-stdio-bridge\production\UserEnvironmentBroadcast.ps1')
...
function New-R5RealProvider([string]$RepoRoot, [string]$StateRoot, [int]$TimeoutSeconds) {
    ...
    $p | Add-Member ScriptMethod RestoreEnv { param($s); foreach ($n in $s.PSObject.Properties.Name) { $i = $s.$n; [Environment]::SetEnvironmentVariable($n, $(if ($i.present) { [string]$i.value } else { $null }), 'User') }; Assert-UserEnvironmentBroadcast }
...
function New-R5FakeProvider([string]$StateRoot, [string]$Scenario = '') {
    $p = [pscustomobject]@{ Kind = 'Fake'; StateRoot = [IO.Path]::GetFullPath($StateRoot); StatePath = Join-Path $StateRoot 'state.json'; ManifestPath = Join-Path $StateRoot 'production-manifest.json'; Journal = 'fake-journal'; TimeoutSeconds = 1; Scenario = $Scenario; Machine = 'machine-baseline'; Env = [ordered]@{}; Marker = $true; PermanentRunning = $true; Canaries = @{}; CanaryAliveAtDiagnosis = $false; DiagnoseInput = ''; PollCount = 0; DiscoverCount = 0; StoppedPids = @(); LauncherPid = 2003; LauncherKillAttempted = $false; Activation = 'DISABLED'; DesktopStarted = $false; HookPass = $true; RoutePass = $true; StockPass = $true; RestoreTrayPass = $true; RestoreLoggingPass = $true; RestoreEnvPass = $true; BroadcastPass = $true; BroadcastCount = 0; BroadcastLog = @(); MachineDrift = $false; Live = $false }
    ...
    $p | Add-Member ScriptMethod Enable { $this.Activation = 'ENABLED'; $this.BroadcastUserEnvironment('enable') }
    $p | Add-Member ScriptMethod Disable { $this.Activation = 'DISABLED'; $this.BroadcastUserEnvironment('disable') }
    ...
    $p | Add-Member ScriptMethod BroadcastUserEnvironment { param([string]$phase); $this.BroadcastCount++; $this.BroadcastLog += $phase; if (!$this.BroadcastPass) { throw 'fake broadcast failed' }; return $true }
    $p | Add-Member ScriptMethod RestoreEnv { param($s); if (!$this.RestoreEnvPass) { throw 'fake env restore failed' }; $this.Env = $s; $this.BroadcastUserEnvironment('restore') }
    ...
    $p | Add-Member ScriptMethod StartDesktop { param($appUserModelId); $this.DesktopStarted = $true; return [ordered]@{ launched = $true; appUserModelId = $appUserModelId } }
